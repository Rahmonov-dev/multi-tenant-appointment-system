...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    List<Appointment> findByEmployementIdAndAppointmentDate(Long serviceId, LocalDate date);
    ...
    @Query("SELECT a FROM Appointment a WHERE a.staff.id = :staffId " +
            "AND a.appointmentDate BETWEEN :startDate AND :endDate " +
            "ORDER BY a.appointmentDate, a.startTime")
    List<Appointment> findByStaffIdAndDateRange(@Param("staffId") Long staffId,
                                                @Param("startDate") LocalDate startDate,
                                                @Param("endDate") LocalDate endDate);
    ...
    @Query("SELECT a FROM Appointment a WHERE a.staff.id = :staffId " +
            "AND a.appointmentDate = :date AND a.status IN :statuses " +
            "ORDER BY a.startTime")
    List<Appointment> findActiveAppointments(@Param("staffId") Long staffId,
                                             @Param("date") LocalDate date,
                                             @Param("statuses") List<AppointmentStatus> statuses);
    ...
    boolean existsByStaffIdAndAppointmentDateAndStartTime(Long staffId,
                                                          LocalDate date,
                                                          LocalTime time);
    ...
    boolean hasTimeConflict(@Param("staffId") Long staffId,
                            @Param("date") LocalDate date,
    ...
    long countByStaffIdAndStatus(Long staffId, AppointmentStatus status);
    ...
    @Query("SELECT a FROM Appointment a WHERE a.staff.id = :staffId " +
            ...
            "AND a.status IN ('PENDING', 'CONFIRMED') " +
            "ORDER BY a.startTime")
    List<Appointment> findTodayAppointmentsByStaff(@Param("staffId") Long staffId);
